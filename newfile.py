...
BOT_TOKEN = "7861895122:AAH3SkJ71qebEemFo1uejySHImGo6b-0fH4"
YEKATERINBURG_TZ = timezone(timedelta(hours=5))
DEFAULT_GROUP = None

# States for ConversationHandler
AWAITING_HOMEWORK, AWAITING_INFO, AWAITING_SPORT, AWAITING_SUGGESTION, AWAITING_HOMEWORK_DELETE, AWAITING_INFO_DELETE, AWAITING_SPORT_DELETE, AWAITING_USER_DELETE, AWAITING_SUGGESTION_DELETE, AWAITING_GROUP, AWAITING_SUGGESTION_RESPONSE, AWAITING_BIRTHDAY, AWAITING_GLOBAL_NOTIFICATION, AWAITING_ADD_SPECIAL_USER, AWAITING_USER_RIGHTS, AWAITING_ROLE_ASSIGNMENT, AWAITING_NOTIFICATION_SETTINGS, AWAITING_CUSTOM_ROLE, AWAITING_TEACHER_SELECTION, AWAITING_TEACHER_PERIOD, AWAITING_ABSENCE_TYPE, AWAITING_ABSENCE_REASON, AWAITING_ABSENCE_PHOTO = range(23)
...
def get_group_role(group_name, role_type):
    ...
    try:
        ...
        print(f"ОШИБКА: Ошибка базы данных при получения роли группы: {e}")
        return None
    finally:
        conn.close()
...
def set_group_role(group_name, role_type, user_id, username, first_name):
    ...
    try:
        ...
        # Обновляем права пользователя в зависимости от роли
        rights_data = {}
        ...
        elif role_type == ROLE_PHYSICAL_ORGANIZER:
            rights_data = {
                'can_send_sport_notifications': 1,
                'can_delete_sport_notifications': 1,
                'role': ROLE_PHYSICAL_ORGANIZER
            }
        elif role_type == ROLE_DEPUTY_PHYSICAL_ORGANIZER:
            rights_data = {
                'can_send_sport_notifications': 1,
                'can_delete_sport_notifications': 1,
                'role': ROLE_DEPUTY_PHYSICAL_ORGANIZER
            }
        ...
        conn.commit()
        print(f"ЛОГ: Установлена роль {role_type} для группы {group_name} - пользователь {first_name}")
        return True
    except sqlite3.Error as e:
...
def remove_group_role(group_name, role_type):
    ...
    if not conn: return False
    try:
        # Получаем пользователя, у которого снимаем роль
        role = get_group_role(group_name, role_type)
        if role and role['user_id']:
            # Сбрасываем права пользователя
            conn.execute('''UPDATE users SET
                         can_send_homework = 0,
                         can_send_notifications = 0,
                         can_send_sport_notifications = 0,
                         can_delete_homework = 0,
                         can_delete_notifications = 0,
                         can_delete_sport_notifications = 0,
                         role = 'student'
        ...
        conn.execute('DELETE FROM group_roles WHERE group_name = ? AND role_type = ?', 
                    (group_name, role_type))
        conn.commit()
        print(f"ЛОГ: Удалена роль {role_type} для группы {group_name}")
        return True
    except sqlite3.Error as e:
...
def fetch_teachers_list():
    """Fetches and updates the list of teachers from schedule data"""
    ...
    try:
        ...
        print(f"ЛОГ: Обновлен список преподавателей. Всего преподавателей: {len(teachers_cache)}")
        return teachers_cache
    except Exception as e:
        print(f"ОШИБКА: Ошибка при обновлении списка преподавателей: {e}")
        return teachers_cache
...
def filter_lessons_by_week(lessons, start_date, end_date):
    """Filters lessons for a specific week range"""
    try:
        filtered = []
...
def filter_lessons_by_teacher(lessons, teacher_name):
    ...
    try:
        ...
        print(f"ОШИБКА: Ошибка при фильтрации по преподавателю: {e}")
        return []
...
def find_merged_groups(current_lesson, all_parsed_lessons):
    """Finds groups that have exactly the same lesson at the same time, room, campus, teacher, subject, and pair number"""
    merged = []
...
def format_schedule(lessons, title, all_parsed_lessons=None):
    """Formats schedule for display with merged groups"""
    try:
        if not lessons:
        ...
        for lesson in lessons:
            try:
                if 'datetime' in lesson:
                    ...
                    # Добавляем информацию о совмещенных группах, если есть
                    if all_parsed_lessons:
                        merged = find_merged_groups(lesson, all_parsed_lessons)
                        if merged:
                            groups_str = ", ".join(merged)
                            lesson_text += f"    👥 Совмещена с группой {groups_str}\n"
...
def format_teacher_schedule(lessons, title, teacher_name):
    """Formats schedule for teacher display with group info"""
    try:
        if not lessons:
        ...
        print(f"ОШИБКА: Ошибка при форматировании расписания преподавателя: {e}")
        return f"❌ Ошибка при формировании расписания: {str(e)}"
...
def get_schedule(period, group_name):
    ...
    try:
        ...
        # Parse all lessons for merged groups (once)
        all_parsed_lessons = []
        for lesson in schedule_data:
            parsed = parse_lesson_data(lesson)
            if parsed and 'datetime' in parsed:
        ...
        if period == '📅 Сегодня':
            ...
            # Находим начало и конец текущей недели (пн-вс)
            start_of_week = today_yekat - timedelta(days=today_yekat.weekday())
            end_of_week = start_of_week + timedelta(days=6)
            
            # Если сегодня выходной (сб, вс) - показываем следующую неделю
            ...
            # Возвращаем РАЗБИТОЕ ПО ДНЯМ расписание
...
def get_teacher_schedule(period, teacher_name):
    """Gets and formats the schedule for a specific teacher"""
    try:
        global schedule_cache, last_schedule_check
        ...
        now = datetime.now(YEKATERINBURG_TZ)
        if last_schedule_check is None or (now - last_schedule_check).total_seconds() > 3600:
            print(f"ЛОГ: Обновляем кэш расписания для преподавателя {teacher_name}")
            schedule_data = fetch_schedule_data()
            if schedule_data:
                schedule_cache = schedule_data
                last_schedule_check = now
        ...
        # Сначала парсим ВСЕ занятия преподавателя
        all_parsed_lessons = []
        for lesson in schedule_data:
        ...
        print(f"ЛОГ: Для преподавателя {teacher_name} найдено {len(all_parsed_lessons)} занятий")
        
        # Теперь фильтруем по периоду
        today_yekat = datetime.now(YEKATERINBURG_TZ).date()
        
        if period == '📅 Сегодня':
            filtered_lessons = []
            for lesson in all_parsed_lessons:
                if lesson['datetime'].date() == today_yekat:
            ...
            filtered_lessons = []
            for lesson in all_parsed_lessons:
                if lesson['datetime'].date() == tomorrow_yekat:
                    filtered_lessons.append(lesson)
        ...
        print(f"ЛОГ: После фильтрации '{period}' осталось {len(filtered_lessons)} занятий")
        if filtered_lessons:
            print(f"ЛОГ: Пример даты: {filtered_lessons[0]['datetime'].strftime('%d.%m.%Y')}")
        
        return filtered_lessons, format_teacher_schedule(filtered_lessons, title, teacher_name)
        
    except Exception as e:
        print(f"ОШИБКА: Произошла ошибка при получения расписания преподавателя: {str(e)}")
...
def split_group_schedule_by_days(lessons, group_name, all_parsed_lessons):
    """Разбивает полное расписание группы на сообщения по дням"""
    try:
        if not lessons:
            return []
        ...
        # Формируем сообщения: одно сообщение = один день
        messages = []
        ...
        print(f"ОШИБКА: Ошибка при разбиении расписания группы по дням: {e}")
        return []

def split_teacher_schedule_by_days(lessons, teacher_name):
    """Разбивает полное расписание преподавателя на сообщения по дням, не разрезая один день"""
    try:
        if not lessons:
            return []
        ...
        print(f"ОШИБКА: Ошибка при разбиении расписания по дням: {e}")
        return []

def get_all_teacher_schedule_by_weeks(teacher_name):
    """Gets all teacher schedule split by weeks"""
    try:
        ...
        # Filter lessons for the specific teacher
        ...
        print(f"ОШИБКА: Ошибка при группировке расписания по неделям: {e}")
        return []

async def check_schedule_changes(context: ContextTypes.DEFAULT_TYPE):
    """Checks for schedule changes and notifies about replacements"""
    ...
    # Если есть старый кэш, сравниваем
    if schedule_cache:
        # Парсим старые и новые данные
        old_parsed = []
        for lesson in schedule_cache:
            p = parse_lesson_data(lesson)
            if p and 'datetime' in p:
                old_parsed.append(p)
        ...
        # Создаём словарь для быстрого доступа по (group, date, start_time)
        old_dict = {}
        for l in old_parsed:
            key = (l['group'], l['datetime'].date(), l['start_time'])
            old_dict[key] = l
        ...
        if changes:
            for change in changes:
                if change[0] == 'changed':
                    ...
                    message = (
                        f"🔄 Изменение в расписании замена :\n\n"
                        f"{old_text}\n\n"
                        f"на :\n\n"
                        f"{new_text}"
                    )
                    ...
                    for user in users:
                        try:
                            ...
                            print(f"ОШИБКА: Не удалось отправить уведомление об изменении: {e}")
...
async def send_daily_schedule(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Parse all lessons for merged groups
    all_parsed = []
    for lesson in schedule_cache:
        p = parse_lesson_data(lesson)
        if p and 'datetime' in p:
...
def format_call_schedule(schedule, day_type):
    ...
    if isinstance(schedule, list):
        # Новый формат с списком
        for item in schedule:
            if item['type'] == 'lesson':
                part_text = " (1-я часть)" if item['part'] == 1 else " (2-я часть)"
                result.append(f"{item['number']} пара{part_text}: {item['start']} - {item['end']}")
            else:  # break
                result.append(f"🕓 Перемена: {item['start']} - {item['end']} ({item['duration']} мин)")
    else:
        ...
        for key, value in schedule.items():
            if isinstance(key, int):  # Lesson
                ...
                if 'duration' in value:
                    result.append(f"🕓 Перемена: {value['start']} - {value['end']} ({value['duration']} мин)")
                else:
                    result.append(f"🕓 Перемена: {value['start']} - {value['end']}")
...
async def show_notification_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        keyboard = [
            ...
            [InlineKeyboardButton(f"🏃 Спортивные уведомления: {'✅' if settings['sport_notifications'] else '❌'}", callback_data='toggle_sport_notifications')],
    ...
    try:
        ...
        if query.data in setting_map:
            ...
            if update_user_notification_settings(user_id, setting_map[query.data], new_value):
                ...
                current_settings = get_user_notification_settings(user_id)
                keyboard = [
                    [InlineKeyboardButton(f"🔔 Уведомления о начале пары: {'✅' if current_settings['lesson_start'] else '❌'}", callback_data='toggle_lesson_start')],
                    [InlineKeyboardButton(f"📝 Уведомления о новом ДЗ: {'✅' if current_settings['new_homework'] else '❌'}", callback_data='toggle_new_homework')],
                    [InlineKeyboardButton(f"📢 Важные уведомления: {'✅' if current_settings['important_notifications'] else '❌'}", callback_data='toggle_important_notifications')],
                    [InlineKeyboardButton(f"🏃 Спортивные уведомления: {'✅' if current_settings['sport_notifications'] else '❌'}", callback_data='toggle_sport_notifications')],
                    [InlineKeyboardButton(f"📅 Расписание на завтра вечером: {'✅' if current_settings['tomorrow_schedule'] else '❌'}", callback_data='toggle_tomorrow_schedule')],
                    [InlineKeyboardButton(f"🌅 Расписание на сегодня утром: {'✅' if current_settings['today_schedule'] else '❌'}", callback_data='toggle_today_schedule')],
                    [InlineKeyboardButton("✅ Готово", callback_data='done_notification_settings')]
                ]
...
async def show_teachers_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Shows a list of all teachers alphabetically (one per row)"""
    try:
        ...
        # КНОПКИ ПО ОДНОЙ В РЯД (каждый преподаватель на отдельной строке)
        keyboard = []
        for teacher in teachers:
            keyboard.append([teacher])
        ...
        return AWAITING_TEACHER_SELECTION  # Изменено на новое состояние
    except Exception as e:
        print(f"ОШИБКА: Ошибка в show_teachers_list: {e}")
        await update.message.reply_text("❌ Произошла ошибка при загрузке списка преподавателей.")
        return ConversationHandler.END

async def handle_teacher_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles teacher selection and shows period menu"""
    try:
        ...
        if text not in teachers:
            ...
            return AWAITING_TEACHER_SELECTION  # Возвращаемся к выбору преподавателя
        
        # Save teacher name in context
        ...
        await update.message.reply_text(
            ...
            f"Выберите период для просмотра расписания:",
            parse_mode='HTML',
            reply_markup=reply_markup
        )
        return AWAITING_TEACHER_PERIOD  # Переходим к выбору периода
    except Exception as e:
        print(f"ОШИБКА: Ошибка в handle_teacher_selection: {e}")
...
async def handle_teacher_period_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles teacher schedule period selection and exits conversation"""
    try:
        text = update.message.text
        ...
        if text == '🔙 Назад':
            # Возвращаемся к списку преподавателей
        ...
        if text not in ['📅 Сегодня', '📅 Завтра', '📅 Неделя', '📅 Все расписание']:
            ...
            return AWAITING_TEACHER_PERIOD  # Остаемся в выборе периода
        ...
        if text == '📅 Все расписание':
            ...
            if not filtered_lessons:
                ...
                # Разбиваем по дням (каждое сообщение = один день)
                day_messages = split_teacher_schedule_by_days(filtered_lessons, teacher_name)
                if day_messages:
                    for i, msg in enumerate(day_messages, 1):
                        await update.message.reply_text(
                            msg,
                            parse_mode='HTML'
                        )
            ...
            # Сегодня/Завтра/Неделя — обычный вывод
            filtered_lessons, schedule_text = get_teacher_schedule(text, teacher_name)
            if filtered_lessons is None:
                await update.message.reply_text(schedule_text, parse_mode='HTML')
            else:
                await update.message.reply_text(schedule_text, parse_mode='HTML')
        
        # ВАЖНО: Выходим из ConversationHandler и показываем главное меню
        await show_main_menu(update, update.effective_user.id)
        return ConversationHandler.END
        
    except Exception as e:
        print(f"ОШИБКА: Ошибка в handle_teacher_period_selection: {e}")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Проверяем, есть ли у пользователя группа в актуальном списке
...
async def show_main_menu(update: Update, user_id: int):
    ...
    try:
        ...
        # Получаем информацию о ролях в группе для динамического отображения кнопок
        ...
        fizorg = get_group_role(group_name, ROLE_PHYSICAL_ORGANIZER) if group_name else None
        deputy_fizorg = get_group_role(group_name, ROLE_DEPUTY_PHYSICAL_ORGANIZER) if group_name else None
        ...
        if user_id == DEVELOPER_ID:
            ...
            keyboard = [
                ...
                (['📞 Связаться с физоргом'] if fizorg else []) +
                (['📞 Связаться с замом физорга'] if deputy_fizorg else []),
                ['🎂 Установить день рождения', '🎓 Сменить группу', '🔧 Настройки уведомлений'],
                ['😊 Я...']
            ]
        else:
...
async def handle_simple_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Проверяем только наличие группы, не проверяем наличие в ALL_GROUPS
        if not user_data or not user_data['group_name']:
            add_user(user_id, user.username, user.first_name, user.last_name)
            return await ask_user_group(update, context)
        ...
        if text == '📅 Расписание':
        ...
        elif text == '👨‍🏫 Расписание преподавателей':
            # Запускаем ConversationHandler для выбора преподавателя
            return await show_teachers_list(update, context)
        ...
        elif text == 'Удалить спортивные уведомления' and can_delete_sport_notifications(user_id, group_name):
        ...
        elif text == '🏃 Отправить спортивное уведомление' and can_send_sport_notifications(user_id, group_name):
            await ask_sport_notification_details(update, context)
        
        elif text == '💡 Предложить идею':
            # Проверяем, разработчик ли пользователь
            if user_id == DEVELOPER_ID:
                await ask_suggestion_details(update, context)
            else:
                # Для всех остальных кнопка не должна быть видна, но на всякий случай игнорируем
                pass
            ...
            # Запуск меню отсутствия
            await ask_absence_type(update, context)

        else:
            # Пересылаем сообщение разработчику, если оно не обработано
...
async def forward_message_to_developer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Пересылает сообщение разработчику"""
    try:
        user = update.effective_user
        user_id = user.id
        text = update.message.text
        ...
        await update.message.reply_text(
            "✅ Ваше сообщение было переслано разработчику. Спасибо за обращение!\n\n/start, если отсутствует интерфейс.",
            reply_markup=ReplyKeyboardRemove()
        )
        
        print(f"ЛОГ: Сообщение от пользователя {user_id} переслано разработчику")
...
async def handle_schedule_period(update: Update, context: ContextTypes.DEFAULT_TYPE, group_name: str):
    ...
    try:
        ...
        if text == '📅 Все расписание':
            # Получаем расписание разбитое по дням
            result = get_schedule(text, group_name)
            
            # Проверяем, вернулся ли кортеж (None, messages)
            if isinstance(result, tuple) and result[0] is None:
                ...
                # Если вернулась строка с ошибкой или обычное расписание
...
async def handle_suggestion_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if text and text.strip() == '✅ Готово':
            ...
            # Отправляем уведомление только разработчику
            try:
                if media_collection:
                    for media_item in media_collection:
                ...
                print(f"Не удалось отправить предложение разработчику: {e}")
...
async def handle_suggestion_response_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles the suggestion response callback (только для разработчика)"""
    try:
        query = update.callback_query
        await query.answer()
        ...
        user_id = query.from_user.id
        if user_id != DEVELOPER_ID:
            await query.answer("❌ У вас нет прав для ответа на предложения.", show_alert=True)
            return
...
async def handle_suggestion_response_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles the suggestion response input (только для разработчика)"""
    try:
        user_id = update.effective_user.id
        user_data = get_user(user_id)
...
async def ask_absence_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Shows absence type menu"""
    try:
        user_id = update.effective_user.id
        user_data = get_user(user_id)
        ...
        await update.message.reply_text(
            "😊 <b>Сообщить о своём статусе</b>\n\n"
            "Выберите один из вариантов:",
            parse_mode='HTML',
            reply_markup=reply_markup
        )
        return AWAITING_ABSENCE_TYPE
    except Exception as e:
        print(f"ОШИБКА: Ошибка в ask_absence_type: {e}")
        return ConversationHandler.END

async def handle_absence_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles absence type selection"""
    try:
        user_id = update.effective_user.id
        text = update.message.text
        ...
        if text not in ['😓 Я опоздаю', '🚫 Я не приду', '🤒 Я заболел']:
            await update.message.reply_text("❌ Пожалуйста, выберите вариант из меню.")
            return AWAITING_ABSENCE_TYPE
        
        context.user_data['absence_type'] = text
        
        if text == '🤒 Я заболел':
            # Сразу отправляем уведомление
            await send_absence_notification(update, context, user_id, text, reason=None, photo=None)
            await show_main_menu(update, user_id)
            return ConversationHandler.END
        else:
            await update.message.reply_text(
                "📝 <b>Укажите причину</b>\n\n"
                "Напишите причину опоздания или неявки.\n"
                "При желании можете прикрепить фото справки.\n\n"
                "Когда закончите, нажмите '✅ Готово'.",
                parse_mode='HTML',
                reply_markup=ReplyKeyboardMarkup([['❌ Отмена'], ['✅ Готово']], resize_keyboard=True)
            )
            # Очищаем предыдущие данные
            context.user_data.pop('absence_reason', None)
            context.user_data.pop('absence_photo', None)
            return AWAITING_ABSENCE_REASON
    except Exception as e:
        print(f"ОШИБКА: Ошибка в handle_absence_type: {e}")
        return AWAITING_ABSENCE_TYPE

async def handle_absence_reason(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles absence reason input"""
    try:
        user_id = update.effective_user.id
        text = update.message.text if update.message.text else (update.message.caption if update.message.caption else "")
        ...
        if text and text.strip() == '✅ Готово':
            reason = context.user_data.get('absence_reason', '')
            photo = context.user_data.get('absence_photo')
            
            if not reason:
                await update.message.reply_text("❌ Пожалуйста, укажите причину.")
                return AWAITING_ABSENCE_REASON
            
            absence_type = context.user_data.get('absence_type')
            if not absence_type:
                await update.message.reply_text("❌ Ошибка: тип отсутствия не выбран.")
                return ConversationHandler.END
            
            await send_absence_notification(update, context, user_id, absence_type, reason, photo)
            await show_main_menu(update, user_id)
            return ConversationHandler.END
        
        if update.message.photo:
            photo = update.message.photo[-1]
            context.user_data['absence_photo'] = photo.file_id
            if text:
                context.user_data['absence_reason'] = text
            await update.message.reply_text("✅ Фото добавлено. Нажмите '✅ Готово', чтобы отправить уведомление.")
            return AWAITING_ABSENCE_REASON
        
        if text and text.strip():
            context.user_data['absence_reason'] = text
            await update.message.reply_text("✅ Причина сохранена. Отправьте фото (опционально) или нажмите '✅ Готово'.")
            return AWAITING_ABSENCE_REASON
        
        await update.message.reply_text("❌ Пожалуйста, укажите причину.")
        return AWAITING_ABSENCE_REASON
    except Exception as e:
        print(f"ОШИБКА: Ошибка в handle_absence_reason: {e}")
        return AWAITING_ABSENCE_REASON

async def send_absence_notification(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, absence_type: str, reason: str = None, photo: str = None):
    """Sends absence notification to starosta"""
    try:
        user = update.effective_user
        user_data = get_user(user_id)
        group_name = user_data['group_name']
        ...
        starosta = get_group_role(group_name, ROLE_STAROSTA)
        if not starosta:
            await update.message.reply_text("❌ В вашей группе не назначен староста. Уведомление не отправлено.")
            return
        
        # Формируем текст
        if absence_type == '🤒 Я заболел':
            text = f"🏥 Студент {user.first_name} заболел."
        elif absence_type == '😓 Я опоздаю':
            text = f"⏰ Студент {user.first_name} опоздает по причине: {reason}"
        else:  # '🚫 Я не приду'
            text = f"❌ Студент {user.first_name} не придёт по причине: {reason}"
        
        # Отправляем уведомление старосте
        if photo:
            await context.bot.send_photo(
                chat_id=starosta['user_id'],
                photo=photo,
                caption=text,
                parse_mode='HTML'
            )
        ...
        await update.message.reply_text("✅ Уведомление отправлено старосте.")
    except Exception as e:
        print(f"ОШИБКА: Ошибка в send_absence_notification: {e}")
        await update.message.reply_text("❌ Не удалось отправить уведомление старосте.")
...
async def init_schedule_cache(application):
    ...
    try:
        ...
        fetch_teachers_list()
        print("ЛОГ: Список преподавателей инициализирован")
    except Exception as e:
    ...
    try:
        ...
        conv_handler = ConversationHandler(
            entry_points=[
                ...
                MessageHandler(filters.Regex('^👨‍🏫 Расписание преподавателей$'), show_teachers_list),
                MessageHandler(filters.Regex('^😊 Я...$'), ask_absence_type),
            ],
            states={
                AWAITING_GROUP: [
                ...
                AWAITING_TEACHER_SELECTION: [
                    MessageHandler(filters.TEXT & ~filters.COMMAND, handle_teacher_selection),
                ],
                AWAITING_TEACHER_PERIOD: [
                    MessageHandler(filters.Regex('^(📅 Сегодня|📅 Завтра|📅 Неделя|📅 Все расписание|🔙 Назад)$'), handle_teacher_period_selection)
                ],
                AWAITING_ABSENCE_TYPE: [
                    MessageHandler(filters.Regex('^(😓 Я опоздаю|🚫 Я не приду|🤒 Я заболел|🔙 Назад)$'), handle_absence_type)
                ],
                AWAITING_ABSENCE_REASON: [
                    MessageHandler(filters.TEXT | filters.PHOTO | filters.VIDEO | filters.Document.ALL, handle_absence_reason)
                ],
                AWAITING_ABSENCE_PHOTO: [
                    MessageHandler(filters.TEXT | filters.PHOTO | filters.VIDEO | filters.Document.ALL, handle_absence_reason)
                ],
            },
            fallbacks=[
